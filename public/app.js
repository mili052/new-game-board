...
function rankingPlatform(product) {
  const status = String(product.status || "").toLowerCase();
  if (/ios|苹果/.test(status)) return "IOS";
  if (/微小|微信|抖小|抖音|小游戏/.test(status)) return "微小";
  if (/app/.test(status)) return "IOS";
  const platform = String(product.platform || "").toLowerCase();
  if (/微小|微信|抖小|抖音|小游戏/.test(platform)) return "微小";
  if (/ios|app|苹果/.test(platform)) return "IOS";
  return String(product.platform || "");
}

function rankingPlatformLabel(product) {
  const platform = rankingPlatform(product);
  return { 微小: "微信小游戏", 抖小: "抖音小游戏", IOS: "iOS", TapTap: "TapTap" }[platform] || platform || "平台待补充";
}
...
function renderRankingPage() {
  ...
  const filtered = allRanking.filter(product => {
    ...
    return (!query || text.includes(query.toLowerCase())) && (!platform || rankingPlatform(product) === platform) && (!genre || primaryCategory(product) === genre);
...
function renderBoard() {
  ...
  previousProducts.forEach(product => splitTags(String(product.topic || "").replace(/[、，,]/g, "/")).forEach(topic => { previousTopicCount[topic] = (previousTopicCount[topic] || 0) + 1; }));
  const topicNames = Object.keys(topicCount).sort((a, b) => topicCount[b] - topicCount[a]).slice(0, 12);
  const topicList = topicNames.map(topic => { const count = topicCount[topic]; const previous = previousTopicCount[topic] || 0; const change = count - previous; const ratio = monthProducts.length ? Math.round(count / monthProducts.length * 100) : 0; return `<div class="topic-row"><b>${escapeHtml(topic)}</b><i><span style="width:${Math.max(5, ratio)}%"></span></i><strong>${count}</strong><em>${ratio}% ${change > 0 ? `↑${change}` : change < 0 ? `↓${Math.abs(change)}` : "-"}</em></div>`; }).join("");
